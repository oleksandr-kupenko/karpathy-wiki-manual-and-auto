...
Configure in .cursor/hooks.json:
{
    ...
    "hooks": {
        "sessionStart": [{
            "command": "uv run --directory karpathy-wiki-manual-and-auto python hooks/cursor/session-start.py"
        }]
    }
}
...
# hooks/cursor/session-start.py → hooks/cursor/ → hooks/ → karpathy-wiki-manual-and-auto/
